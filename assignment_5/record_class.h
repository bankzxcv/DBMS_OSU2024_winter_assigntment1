...
/* This record_class.h contains the class Records, which can be used to store
tuples form Emp.csv (stored in the EmpRecords) and Dept.csv (Stored in
DeptRecords) files.
*/
#include <bits/stdc++.h>
...
class Records {
 public:
  ...
  string joinString = "";
  string runName = "";
  streampos savePointIndex = 0;
  streampos index;
  int currentMPage = 0;

  int no_values =
      0;  // You can use this to check if you've don't have any more tuples
  int number_of_emp_records =
      0;  // Tracks number of emp_records you have on the buffer
  int number_of_dept_records =
      0;  // Track number of dept_records you have on the buffer
  void print() {
...
// Grab a single block from the Emp.csv file and put it inside the EmpRecord
// structure of the Records Class
Records Grab_Emp_Record(fstream &empin) {
  ...
  if (getline(empin, line, '\n')) {
    ...
    // Ensuring that you cannot use both structure (EmpEecord, DeptRecord) at
    // the same memory block / time
...
// Grab a single block from the Dept.csv file and put it inside the DeptRecord
// structure of the Records Class
